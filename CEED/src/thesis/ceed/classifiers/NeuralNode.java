...
public class NeuralNode extends NeuralConnection {
	...
	/**
	 * @param id
	 *            The string name for this node (used to id this node).
	 * @param r
	 *            A random number generator used to generate initial weights.
	 * @param m
	 *            The methods this node should use to update.
	 */
	...
	public double outputValue(boolean calculate) {
		if (Double.isNaN(m_unitValue) && calculate) {
			...
			NeuralConnection[] inputs = getInputs();
			double value = weights[0];
			for (int noa = 0; noa < getNumInputs(); noa++) {
	...
	public double errorValue(boolean calculate) {
		if (!Double.isNaN(m_unitValue) && Double.isNaN(m_unitError) && calculate) {
			NeuralConnection[] outputs = getOutputs();
			int[] oNums = getOutputNums();
			double error = 0;

			for (int noa = 0; noa < getNumOutputs(); noa++) {
				error += outputs[noa].errorValue(true)
						* outputs[noa].weightValue(oNums[noa]);
			}
			double value = outputValue(false);
			error *= value * (1 - value);
	 ...
	 * 
	 * @param n
	 *            The connection number to get the weight for, -1 if The
	...
	/**
	 * call this function to get the weights array. This will also allow the
	...
	/**
	 * call this function to get the change in weights array. This will also
	 * allow the change in weights to be updated.
	 * 
	...
	public void updateWeights(double l, double m) {
		if (!m_weightsUpdated && !Double.isNaN(m_unitError)) {
			NeuralConnection[] inputs = getInputs();
			double[] cWeights = getChangeInWeights();
			double[] weights = getWeights();
			
			double learnTimesError = 0;
			learnTimesError = l * errorValue(false);
			
			double c = learnTimesError + m * cWeights[0];
			weights[0] += c;
			cWeights[0] = c;

			int stopValue = getNumInputs() + 1;
			...
			// required to update the weights at this node (while the error and
			// output value ao need to be recursively calculated)
	 ...
	 *            The unit.
	 * @param n
	 *            It's connection number for this connection.
	...
	protected boolean connectInput(NeuralConnection i, int n) {
		...
		// it leaves the num inputs aligned on the space the weight needs to go.
	...
	/**
	 * This will allocate more space for input connection information if the
	 * arrays for this have been filled up.
	 */
	 ...
	 * @param n
	 *            The connection number at the other end, -1 if all the
	 *            connections to this unit should be severed (not the same as
	 *            removeAllInputs).
		...
		do {
			...
			if (loc >= 0) {
				for (int noa = loc + 1; noa < m_numInputs; noa++) {
					...
					m_inputList[noa - 1].changeOutputNum(m_inputNums[noa - 1],
							noa - 1);
				}
				m_numInputs--;
				removed = true;
	...
	/**
	 * This function will remove all the inputs to this unit. In doing so it
